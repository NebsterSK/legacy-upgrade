 ...
 */
export function UpgradeArrow({ className, style }: GlyphProps) {
    return (
        <svg viewBox="0 0 100 100" className={className} style={style} aria-hidden fill="currentColor">
...
/**
 * The logo square: arrow knocked out of a brand-blue tile. The corner radius is a share of
 * the tile's size, so the mark keeps its shape at every size; pass bg / text classes to
 * invert it (the phone menu sets it white on the brand drench).
 */
export function UpgradeMark({ className }: { className?: string }) {
    return (
        <span
            className={cn(
                'bg-brand text-brand-foreground inline-flex items-center justify-center rounded-[10%]',
                className
            )}
            aria-hidden
        >
...
export function EuroGlyph({ className, style }: GlyphProps) {
    return (
        <svg viewBox="0 0 100 100" className={className} style={style} aria-hidden fill="currentColor">
